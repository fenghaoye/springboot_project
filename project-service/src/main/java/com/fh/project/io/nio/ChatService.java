...
//                continue;
//            }
            System.out.println("调用 selector.select 方法");
            // 添加事件（每次连接，读写时都会创建事件），这个方法可能有bug，
            // 会导致该方法一直返回为0
            selector.select(timeout);

//            以下代码为了规避selector.select(timeout)方法产生的bug
            ...
            if(count>=10){
                ...
                rebuildSelector();
                count=0;
                selector.selectNow(); // 立即返回，不会有timeout
                continue;
            }
    ...
    private void rebuildSelector() throws IOException {
        ...
        // 将oldSelect上的事件重新注册到newSelector
