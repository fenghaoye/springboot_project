...
public class InnerClassTest1 {
    ...
    static int outField5 = 5;
    InnerClassA innerObj;

    public InnerClassTest1() {
        // 在外部类对象内部，直接通过 new InnerClass(); 创建内部类对象
        // 下面的代码中外部类对象无法获得内部类对象的引用，
        // InnerClassA innerObj = new InnerClassA();
        // 外部类要想获得内部类对象的引用，必须在外部类申明一个属性InnerClassA innerObj ;
        innerObj = new InnerClassA();
...
//            System.out.println("其外部类的 field1 字段的值为: " + field1); // 编译错误！！
    ...
    public static void main(String[] args) {
        ...
        // 内部类对象本身会有一个隐藏属性this$0用来保存此内部类对象所属的外部类对象的引用
        InnerClassA innerObj = outerObj.new InnerClassA();
        InnerClassA innerObj2 = outerObj.new InnerClassA();
        System.out.println("");
    }
